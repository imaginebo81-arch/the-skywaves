...
export default function CoursesPage() {
  ...
  const filteredCourses = activeCategory === "All" 
    ...
    ? coursesData.filter(c => !["English", "Fashion", "Boutique"].includes(c.category))
    : coursesData.filter(c => c.category === activeCategory || c.category.includes(activeCategory));
    ...
    <div className="w-full max-w-[1400px] mx-auto px-4 md:px-12 py-12 flex flex-col gap-8">
      ...
      <div className="text-center max-w-2xl mx-auto mb-8">
        ...
        <p className="text-gray-600 text-lg">
          Master the technologies driving tomorrow. Explore our diverse range of computer courses designed for everyone.
        </p>
      </div>
          ...
          <div key={course.id} className={`bento-card overflow-hidden flex flex-col border border-gray-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 rounded-2xl group ${courseGradients[index % courseGradients.length]}`}>
            ...
            <div className="p-6 flex flex-col flex-grow gap-4">
              ...
              <div className="mt-auto pt-4">
                <button className="btn-primary w-full py-3 text-sm cursor-pointer relative overflow-hidden flex justify-center items-center group/btn">
