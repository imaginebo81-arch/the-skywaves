...
export default function EnglishCourses() {
  const englishCourses = coursesData.filter(c => c.category === "English").slice(0, 4);

  return (
    <section className="w-full max-w-[1400px] mx-auto px-4 md:px-12 pb-16">
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Basic to Advance English Courses</h2>
          <p className="text-gray-600">Enhance your language skills for better communication and confidence.</p>
        </div>
        <Link 
          to="/courses"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {englishCourses.map((course, index) => (
          <div key={course.id} className={`bento-card overflow-hidden flex flex-col border border-gray-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 rounded-2xl group ${courseGradients[(index + 4) % courseGradients.length]}`}>
            ...
            <div className="p-5 flex flex-col flex-grow gap-3">
              ...
              <div className="mt-auto pt-4">
                <button className="btn-primary w-full py-2.5 text-sm flex justify-center items-center gap-2 relative overflow-hidden font-bold group/btn cursor-pointer">
