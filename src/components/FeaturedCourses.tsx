...
export default function FeaturedCourses() {
  const { featured, marketingCourses, courseGradients } = useContent();
  const { openEnroll } = useEnroll();
  const courses = marketingCourses.slice(0, 4);
          ...
          <div key={course.id} className={`bento-card overflow-hidden flex flex-col border border-gray-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 rounded-2xl group ${courseGradients[index % courseGradients.length]}`}>
            ...
            <div className="p-6 flex flex-col flex-grow gap-4">
              ...
              <div className="mt-auto pt-4">
                <button onClick={() => openEnroll(course.academicCourseId)} className="btn-primary w-full py-3 text-sm flex justify-center items-center gap-2 relative overflow-hidden group/btn cursor-pointer">
