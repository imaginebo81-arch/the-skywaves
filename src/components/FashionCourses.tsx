...
export default function FashionCourses() {
  const fashionCourses = coursesData.filter(c => c.category === "Fashion").slice(0, 4);

  return (
    <section className="w-full max-w-[1400px] mx-auto px-4 md:px-12 pb-16">
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Fashion Designing</h2>
          <p className="text-gray-600">From concept sketches to runway execution.</p>
        </div>
        <Link 
          to="/courses"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {fashionCourses.map((course, index) => (
          <div key={course.id} className={`bento-card overflow-hidden flex flex-col border border-gray-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 rounded-2xl group ${courseGradients[(index + 8) % courseGradients.length]}`}>
